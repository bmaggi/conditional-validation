...
public class Not extends AbstractEvaluable implements IEvaluable{

	public boolean evaluate() throws MojoExecutionException{
		if (evaluableList.size() != 1) {
			throw new MojoExecutionException("Negation is only working on one element");
		}
		return !evaluableList.get(0).evaluate();
	}
